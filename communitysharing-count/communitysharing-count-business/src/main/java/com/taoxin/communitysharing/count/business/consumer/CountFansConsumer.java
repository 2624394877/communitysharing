...
@RocketMQMessageListener(
        consumerGroup = "communitysharing_group_" + MQConstant.FANS_COUNT,
        topic = MQConstant.FANS_COUNT
)
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        for (Map.Entry<Long, List<CountFollowUnfollowMqDTO>> entry : groupMap.entrySet()) {
            ...
            countMap.forEach((k, v) -> {
                // Redis Key
                String redisKey = RedisKeyConstant.buildCountUserKey(k);
                ...
                if (isExisted) {
                    ...
                    redisTemplate.opsForHash().increment(redisKey, RedisKeyConstant.FIELD_FANS_TOTAL, v);
                }
            });

            // 放入数据库
            Message<String> message = MessageBuilder.withPayload(JsonUtil.toJsonString(countMap))
                    .build();
            // 异步发送 MQ 消息，提升接口响应速度
