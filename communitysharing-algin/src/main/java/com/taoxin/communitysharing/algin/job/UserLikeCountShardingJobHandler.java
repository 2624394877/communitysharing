...
@Slf4j
@Component
public class UserLikeCountShardingJobHandler {
    @Resource
    ...
    @XxlJob("userGetLikeCountShardingJobHandler")
    public void userGetLikeCountShardingJobHandler() throws Exception {
        // 分片处理逻辑
        // 序号
        ...
        while (true) {
            List<Long> userIdList = selectMapper.selectBatchFromDataAlignUserLikeCountTempTable(tableNameSuffix, batchSize);
            log.info("【用户获得点赞数据对齐】开始对齐数据,tableNameSuffix: {}，userIdList：{}" ,tableNameSuffix, userIdList);
            if (CollUtil.isEmpty(userIdList)) break;
            userIdList.forEach(userId -> {
                int likeTotal = selectMapper.selectCountFromUserLikeTableByUserId(userId);
                int count = updateMapper.updateUserCountLikeTotal(userId, likeTotal);
                log.info("【用户获得点赞数据对齐】开始对齐数据，数据库更新{}条记录", count);
                if (count > 0) {
                    log.info("【用户获得点赞数据对齐】数据库对齐成功，正在进行缓存。。。。");
                    ...
                    log.info("【用户获得点赞数据对齐】{}缓存是否存在：{}", userCountZSetKey, userCountZSetExist);
                    if (userCountZSetExist) {
                        redisTemplate.opsForZSet().add(userCountZSetKey, RedisKeysConstant.LIKE_TOTAL, likeTotal);
                        log.info("【用户获得点赞数据对齐】缓存更新成功");
                    }
                }
            });
            deleteMapper.batchDeleteDataAlignUserLikeCountTempTable(tableNameSuffix, userIdList);
