...
public class FansCountShardingXxlJob {
    ...
    public void fansCountShardingJobHandler() throws Exception {
        ...
        while (true) {
            List<Long> userIdList = selectMapper.selectBatchFromDataAlignFansCountTempTable(tableNameSuffix, batchSize);
            log.info("【用户粉丝数据对齐】开始对齐数据,tableNameSuffix: {}，userIdList：{}" ,tableNameSuffix, userIdList);
            if (CollUtil.isEmpty(userIdList)) break;
            userIdList.forEach(userId -> {
                int fansTotal = selectMapper.selectCountFromFansTableByUserId(userId);
                int count = updateMapper.updateUserCountFansTotal(userId, fansTotal);
                log.info("【用户粉丝数据对齐】开始对齐数据，数据库更新{}条记录", count);
                if (count > 0) {
                    log.info("【用户粉丝数据对齐】数据库对齐成功，正在进行缓存。。。。");
                    ...
                    if (userCountZSetExist) {
                        redisTemplate.opsForZSet().add(userCountZSetKey, RedisKeysConstant.FNAS_TOTAL, fansTotal);
