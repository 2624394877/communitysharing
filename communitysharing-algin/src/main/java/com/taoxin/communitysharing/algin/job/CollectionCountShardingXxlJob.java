...
@Slf4j
@Component
public class CollectionCountShardingXxlJob {
    @Resource
    ...
    @XxlJob("collectCountShardingJobHandler")
    public void collectCountShardingJobHandler() throws Exception {
        // 分片处理逻辑
        // 序号
        ...
        while (true){
            List<Long> contentIdList = selectMapper.selectBatchFromDataAlignCollectCountTempTable(tableNameSuffix, batchSize);
            log.info("【内容收藏数据对齐】开始对齐数据,tableNameSuffix: {}，contentIdList：{}" ,tableNameSuffix, contentIdList);
            if (CollUtil.isEmpty(contentIdList)) break;
            contentIdList.forEach(contentId -> {
                int collectTotal = selectMapper.selectCountFromCollectTableByContentId(contentId);
                int count = updateMapper.updateContentCountCollectTotal(contentId, collectTotal);
                log.info("【内容收藏数据对齐】开始对齐数据，数据库更新{}条记录", count);
                if (count > 0) {
                    log.info("【内容收藏数据对齐】数据库对齐成功，正在进行缓存。。。。");
                    ...
                    if (contentCountZSetExist) {
                        redisTemplate.opsForHash().put(contentCountZSetKey, RedisKeysConstant.COLLECTED_TOTAL, collectTotal);
                        log.info("【内容收藏数据对齐】{}缓存成功。。。。",contentCountZSetKey);
                    }
                }
                // 索引更新
